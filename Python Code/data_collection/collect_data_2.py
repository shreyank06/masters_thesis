...
class CsvCollector:
    ...
    def fetch_and_save_csv(self):
        ...
        #     results_csv_filename = f"per_ue_{self.component}_needed.csv"
        ...
        print(f"Results CSV file saved as: {csv_filename} in 'data' folder.")


    def collect_csv_data(self):
            self.df = fetch_and_convert_data(self.config, 'queries', self.start_time, self.end_time, self.config['step'], self.component)
            ...
            # # csv_file_path = os.path.join(component_folder_path, f"{self.registration_number}_{self.ops_per_second}_{0}{file_suffix}_{self.component}.csv")
