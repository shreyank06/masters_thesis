...
def main():
    ...
    while True:
        ...
        print(f"Training model for time range: {config['start_time']} to {end_time}")

        if(config["retrain_model"]):
            my_model = retrain_model(trainX, trainY, testX, testY)
        ...
        subscriber_count_predicted_value = scaler.inverse_transform(np.reshape(prediction_copies_array, (len(prediction), 11)))[:, 3]

        #Print the reshaped and inverse-transformed predicted values
        print("Predicted Values -- ", subscriber_count_predicted_value)

        # Continue with the rest of the code
        original_copies_array = np.repeat(testY, 11, axis=-1)
        original = scaler.inverse_transform(np.reshape(original_copies_array, (len(testY), 11)))[:, 3]
